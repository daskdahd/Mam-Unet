...
if __name__ == "__main__":
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(VOCdevkit_path, "VOC2007_1000/ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_path, "VOC2007_1000/ImageSets/Segmentation/val.txt"),"r") as f:
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # 早停和最佳权重保存逻辑
            if local_rank == 0:
                current_miou = eval_callback.get_miou()
                ...
                if patience_counter >= early_stopping_patience and current_miou > 0:
                    print(f"🛑 触发早停! 连续{early_stopping_patience}轮mIoU未提升")
                    break
    ...
    if local_rank == 0:
        ...
        # 进行完整的FPS性能测试
        print("\n🚀 开始模型FPS性能测试...")
        from utils.utils_fit import test_model_fps
        
        # 使用最佳权重进行FPS测试
        if os.path.exists(best_weights_path):
            print(f"📂 加载最佳权重进行FPS测试: {best_weights_path}")
            model.load_state_dict(torch.load(best_weights_path, map_location=device))
        
        # 执行FPS测试并保存到log目录
        device = next(model.parameters()).device
        fps_results = test_model_fps(
            model=model, 
            device=device, 
            input_shape=input_shape, 
            log_dir=log_dir,
            model_name=f"{backbone}_UNet",
            test_iterations=100
        )
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            for batch_size, result in fps_results.items():
                if result is not None:
                    f.write(f"Batch Size {batch_size}: {result['fps_avg']:.2f} FPS\n")
        
        print(f"📁 训练总结保存到: {summary_path}")
        print("✅ 所有结果已保存到log目录!")
