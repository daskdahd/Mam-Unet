...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            # 更新当前mIoU
            self.current_miou = temp_miou
            
            # 确保保存mIoU到文件
            miou_log_path = os.path.join(self.log_dir, "epoch_miou.txt")
            with open(miou_log_path, 'a') as f:
                f.write(f"{temp_miou:.2f}\n")
